...
and adds two things the terminal cannot do:

**A graph of the evidence.** The decision on top, the conditions holding it
up, the claims beneath those, and the people who spoke at the base. Drag to
orbit, click a node to fly to it, filter by node type. Views are
hash-addressable (`#/graph`), so a sheet can be linked and survives a reload.

**An assistant, bottom right.** A circular button on every sheet opens a small
chat panel. It answers *from the selected revision only* — its badge and its
header both say which — so the same question against v1 and v4 can legitimately
...
The assistant is handed the whole revision in one request: every claim with its
verbatim support and locator, the derived conclusions, the authority matrix,
and what moved from the previous revision. There is no retrieval step and no
vector index — a revision is about 8k tokens, so there is nothing to retrieve.

It does not reason about the decision. `graph.py` already decided what is met,
unmet, unknown and contested; the model reads that out and cites it.

**Citations are resolved, not trusted.** The model writes claim ids inline; the
server looks each one up in the revision. An id that does not exist comes back
flagged as unsupported rather than rendered as evidence, so a fabricated
citation cannot reach the screen.

Answers stream, so the first words appear rather than a spinner. Measured on
the real API over five identical questions: **median 1.6s to first token, 2.7s
complete**. One run in five cold-started and took 81s, which is provider
variance rather than anything in this code — the client gives up after two
minutes and keeps your question.

Requires `OPENAI_API_KEY` in `.env`.

## What it looks like

Screenshots are of the running app at revision **v4** — the packet plus a
document read in by the agent. They are captured from the real thing, not
mocked up; `docs/screenshots/` holds the originals.

### The decision brief

![The decision brief](docs/screenshots/01-brief.png)

The verdict as a rubber stamp, because that is literally what is missing: the
quote arrives stamped "PRICING SUBMITTED — NOT AUTHORISED", and this is the
stamp Maya Chen has not applied. Above the fold, **what is already true and no
longer preventable** — work performed without authorisation, a cost larger than
the quote, a crew held. Below it, the conditions still ahead of her, each with
the reason spelled out and cited. Diagonal hatching marks anything whose basis
is contested.

### The evidence map

![The evidence map](docs/screenshots/02-graph.png)

The whole revision: every claim, every person, every source — including the
ones cited but never supplied, which render as `[MISSING]`. Claims cluster
around the subject they are about rather than around who said them. The two
`[CONTESTED]` queues are the live conflicts. Claim labels appear on hover or
zoom so the structure is not buried; `conflicts · 2` isolates just the disputes
with one hop of context.

### Claim queues

![Claim queues](docs/screenshots/03-evidence.png)

Every claim grouped by what it is about, latest on top. Nothing is deleted: a
superseded claim stays readable, struck through, with both citations. Each row
carries the verbatim text, the author, and the locator it came from.

### Conflicts

![Conflicts](docs/screenshots/04-conflicts.png)

Where the packet disagrees with itself. The duct offset is stated four
different ways and never measured, so the product prints all four and refuses
to derive a fifth. The head claim wins on recency alone — which is said out
loud, because everything derived from it is marked *premise contested*.

### Unknowns

![Unknowns](docs/screenshots/05-unknowns.png)

What the record does not say — never rendered as "no". A meeting with no
minutes is not a meeting that did not happen. This sheet also lists documents
cited by a claim but absent from the packet, so a missing basis is a state
rather than a silence.

### Authority

![Authority and provenance](docs/screenshots/06-authority.png)

Who can actually decide what, with every capability quoted from the project
primer. This is the sheet that makes the whole product work: Tasha Reed's
support is warm, senior, and contractually worth nothing, and the tool says so.

### Add evidence — the agent

![The ingestion agent](docs/screenshots/07-agent.png)

Drop in any number of files, of any type. They are read inside a disposable
container that identifies each format from its contents rather than its
extension, and the claims become a **new revision branched off the one you have
selected**. While it runs it reports real steps — building the image, reading
documents, the live shell-command count — and the run survives you navigating
to another sheet.

### Revisions

![Revisions and the diff](docs/screenshots/08-revisions.png)

The brief's core requirement: *evidence changed → here is what moved*. Every
row is computed by comparing two derivations, never authored. Selecting a
revision swaps the whole ledger, so v1 shows v1's claims everywhere.

### Verify

![Verify](docs/screenshots/09-verify.png)

Every claim's verbatim support text, checked against the real PDF or workbook
it was read from. This is how the ledger's accuracy is demonstrated rather than
asserted. Image claims are reported as skipped, not silently passed.

### The assistant

![The assistant](docs/screenshots/10-chat.png)

A question about the evidence gets asked while you are looking at something
else, so it floats over every sheet rather than being a place you navigate to.
It answers **from the selected revision only** — the badge and the header both
say which. Claim ids it writes are resolved against the ledger, so a citation
it invented would be flagged as unsupported rather than shown as evidence.

## The same engine in the terminal

The CLI is not a lesser version — it is the same `conclusions()`, and the
condition status strings the browser renders are generated by the terminal
renderer, so the two cannot drift apart.

### `/brief`

![CLI brief](docs/screenshots/11-cli-brief.png)

### `/why cost_authorised`

![CLI why](docs/screenshots/12-cli-why.png)

The full derivation of one condition: the reason, then **every claim the rule
actually read** — id, kind, author, citation and verbatim text. Where a rule
also looked at something it may never gate on, that is shown separately.

### `/conflicts`

![CLI conflicts](docs/screenshots/13-cli-conflicts.png)

### `/verify`

![CLI verify](docs/screenshots/14-cli-verify.png)
...
npm --prefix web test                        # 10 tests, escaping and chat store
...
- **The assistant cannot invent a citation.** Claim ids it writes are resolved
  against the revision; unknown ids are reported as unsupported rather than
  shown. Tested directly, including the case where the model cites a claim that
  does not exist.
...
11. **Chat latency is not under our control.** Typical answers start in under
    two seconds, but the provider occasionally cold-starts and stalls for over
    a minute. One-shot answering and streaming reduce the exposure; they cannot
    remove it. Prompt-cache warming was built, measured and removed — the warm
    call is as slow as the question it was meant to speed up.
12. **The assistant can still be wrong in prose.** Its citations are verified
    and the conclusions it reads are deterministic, but nothing checks that the
    sentence it wrote around them is a fair summary. Every answer is one click
    from the claims it cited, which is the mitigation.
...
| `tests/` | 124 tests, named after the risk each prevents |
| `docs/superpowers/specs/` | the design, written before the code |
| `web/src/components/ChatWidget.vue` | the assistant |
